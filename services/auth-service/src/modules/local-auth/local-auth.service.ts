...
export class UserService {
    ...
    public async createNewAccessCode(phone: string) {
        try {
            ...
            // Reference to the user document using phone number as the ID
            const userRef = db.collection('users').doc(phone);

            // set() with merge: true creates the doc if it doesn't exist, 
            // or overwrites just the accessCode if it does.
            await userRef.set({
                phone,
                accessCode
            }, { merge: true });

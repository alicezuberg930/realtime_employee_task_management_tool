...
export default class Google extends BaseProvider {
    ...
    override async getAccessToken(code: string, codeVerifier: string): Promise<OAuth2Token> {
        ...
        if (!tokenResponse.ok) {
            ...
            throw new BadRequestException(`Google API error: ${error}`)
        }
    ...
    override async fetchUser(tokenData: OAuth2Token): Promise<OauthAccount> {
        ...
        if (!response.ok) {
            ...
            throw new BadRequestException(`Google API error (${response.status}): ${errorText}`)
        }
        const userData: GoogleUserResponse = await response.json() as GoogleUserResponse
        return {

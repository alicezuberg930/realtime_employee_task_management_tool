...
export class JWT<TValue extends Record<string, unknown>> {
    ...
    public async sign(payloadClaims: TValue, options: JWTOptions = {}): Promise<string> {
        const header = {
            alg: this.algorithm,
            typ: 'JWT',
            ...options.headers,
    ...
    public async verify(token: string): Promise<TValue & JWTHeader> {
        ...
        const expectedSignaturePart = encodeBase64Url(new Uint8Array(expectedSignature))

        if (expectedSignaturePart !== signaturePart) throw new Error('Invalid token signature')
        ...
        if (payload.exp && currentTime >= payload.exp) throw new Error('Token has expired')
        if (payload.nbf && currentTime < payload.nbf) throw new Error('Token not valid yet')
